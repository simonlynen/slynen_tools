...
// Code control
static const int nbThreads = 7;
static const double maxSeconds = 30.0;
...
)
{
    ...
    while(i<maxLoops)
    {
        ...
        {
            Node *node = 0;
            if(false==useTry) node = (Node*)lifo.pop();
            else
            {
                while(1)
                {
        ...
        else
        {
            ...
            {
                head = (Node*)node->next;
                if(false==useTry) lifo.push(node);
                else
                {
                    while(1)
                    {
                        ++attemptedPushes;
                        if(lifo.tryPush(node)) break;
                        ++failedPushes;
                    }
                }
                ++successfulPushes;
...
)
{
    ...
    #if defined(__i386__)
        printf("plaftorm: i386\n");
    #elif defined(__x86_64__)
        printf("plaftorm: x86-64\n");
    #else
        #error "????"
    #endif
    printf("\n");
    ...
    for(int i=0; i<n; ++i) lifo.push(nodes[i]=new Node(i));
